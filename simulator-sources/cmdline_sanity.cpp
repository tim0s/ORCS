...
static void process_ptrnargs(IN char *ptrn,
	...
	if ((strcmp(ptrn, "neighbor") == 0 ||
		...
		*ptrnarg_i = strtoi(ptrnarg, &next_num, 10);
		...
		ptrnvsptrn_t *matches = (ptrnvsptrn_t *)calloc(1, sizeof(*matches));	/* Contains the extracted data. */
		if (matches == NULL)
			goto exit;
		 ...
		 * recursively for each of the provided patterns. */
